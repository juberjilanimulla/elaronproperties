...
const usersellpropertyimage = Router();

// MULTI IMAGE UPLOAD
usersellpropertyimage.post("/:id", (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      const sellproperty = await sellpropertymodel.findById(req.params.id);
      if (!sellproperty) {
        req.files.forEach((file) => fs.unlinkSync(file.path));
        return errorResponse(res, 404, "Property not found");
      }

      if (!Array.isArray(sellproperty.images)) {
        sellproperty.images = [];
      }
      ...
      for (const file of req.files) {
        ...
        const s3Key = `sellproperties/${fileName}`;
        ...
        sellproperty.images.push(s3Res.Location); // Add image URL
        fs.unlinkSync(file.path); // Remove temp file
      }

      await sellproperty.save();
      return successResponse(res, "Images uploaded successfully", sellproperty);
    } catch (error) {
...
export default usersellpropertyimage;
