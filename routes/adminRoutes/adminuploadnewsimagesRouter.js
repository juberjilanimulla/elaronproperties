...
}).single("coverimage"); // Support multiple files (max 10)
  ...
  upload(req, res, async (err) => {
    ...
    try {
      ...
      for (const file of req.files) {
        ...
        news.coverimage.push(s3Res.Location); // Add image URL
        fs.unlinkSync(file.path); // Remove temp file
      }

      await news.save();
      return successResponse(res, "coverimage uploaded successfully", news);
    } catch (error) {
