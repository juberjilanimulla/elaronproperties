...
const adminoffplanimages = Router();

// MULTI IMAGE UPLOAD
adminoffplanimages.post("/:id", (req, res) => {
  upload(req, res, async (err) => {
    ...
    try {
      const offplan = await offplanpropertymodel.findById(req.params.id);
      if (!offplan) {
        req.files.forEach((file) => fs.unlinkSync(file.path));
        return errorResponse(res, 404, "offplan not found");
      }

      if (!Array.isArray(offplan.images)) {
        offplan.images = [];
      }
      ...
      for (const file of req.files) {
        ...
        offplan.images.push(s3Res.Location); // Add image URL
        fs.unlinkSync(file.path); // Remove temp file
      }

      await offplan.save();
      return successResponse(res, "Images uploaded successfully", offplan);
    } catch (error) {
...
export default adminoffplanimages;
